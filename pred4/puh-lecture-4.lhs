...
a LIST or a TREE, but it can be any recursive structure. We call such a
recursion STRUCTURAL RECURSION. We use structural recursion to process a data
structure (something we would do with a loop in an imperative language).
...
(What would happen if we were to apply this function to an empy list.)
...
This is how we can implement the filtering of a lsit:
...
function that takes a list and pairs up the consequtive elements would be
defined like this:
...
Let's look at the definition of the functorial function again:
...
>         sum (x:xs) s = sum xs (s `seq` x+s)

> main = do
>  putStr . show $ sum2 [1..10000000]
...
only after the recursive call returs that we can compute the value of the
whole expression. Conceptually, this means that we gradually build up larger
and larger expression, and we can start reducing this expression only we've
...
there is nothing that is waiting to be done AFTER the recursive call is
...
slighly more generic definition:
...
perhaps easier to comprehend the behaviour of such functions (accumulator-style
...
using accumulators only makes sense if they are STRICT. More on this later in
the class.
...
- Now write a standard recrusive definition (without an accumulator).

=== GUARDED RECURSION =========================================================

But sometimes using an accumulator doesn't even make sense to begin with. For
...
allocated during the computation is counted, but here no extra memory allocated
in this case.)
...
Hence we use should standard recursion in this case, too:
...
part of the result, the recursion will never generate results beyond this point
...
* accumulator-style, but only if you need the whole result, and make sure the
  accumulators are strict;
