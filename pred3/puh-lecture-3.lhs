...
IRREFUTABLE. Some patterns, however, are REFUTABLE, i.e., can fail to match.
For example:
...
> head' :: [a] -> a
> head' []    = error "No head to decapitate"
> head' (x:_) = x
...
Can we use pattern patching to accomplish the opposite: split up the initial
...
Also, patterns can be different:
...
It's a bit more nicer with pattern matching:
...
sense that cause an error. If an element doesn't match a pattern, it is simply
skipped.
...
But luckily we will not be doing Prolog today.)

The correct way of doing this is to use different variables and explicitly
check for equality:
...
We should avoid situations in which the same value is computed the many times
over. In Haskell we use local definitions for that.
...
the top level (so that it becomes visible to all functions).
...
These locally-defined functions can also use pattern matching:

> middleOne' x y = (middle x,middle y)
>   where middle (0,0,0) = 1
>         middle (_,w,_) = w

The question that's been tormenting you: when should I use 'where', and when
should I use global definitions instead?

Reasons for using 'where':
...
- Define 'quartiles xs' that returns the quartiles (q1,q2,q3) of a given list.
  ...
  sorted in ascending order. (You can use the built-in 'splitAt' function and
...
As a matter of fact, pattern matching used in function definition is just a
syntactic sugar for a 'case' statement. Otherwise said, every definition of 
the form:
...
The difference is that 'case' can be used everywhere:
...
- Write a function that takes in a pair (a,b) and a list [c] and returns the
  following string:
  "The pair [contains two one's|contains one one|does not contain a single one]
...
* pattern matching in function definition is simlar to 'case'
...
Note: Some tests you can be done with pattern matching (e.g., is the list
empty?), but some require Boolean expressions (e.g., is the number less than
1?)
