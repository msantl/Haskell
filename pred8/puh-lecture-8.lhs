...
words, type constructor 'Maybe' will produce only when given a type as input.
E.g. 'Maybe Int' is a type. Similarly, 'Either' expects two types, before it
produces a type. So these type constructors themselves are of different
"types". We're talking about "types of types" here, which we call KINDS in
Haskell. 'Int', 'Maybe', and 'Either' are type constructors of different types.
You can find out their kind using the ":kind" command in ghci:
