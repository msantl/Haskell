...
models: computation with state, nondeterministic computation, a computation
that can fail, etc. The most prominent example of a monad is the IO monad,
which we use for IO operations.
...
Then follows another computations, that again can fail or succeed, etc. We need
some abstraction machinery to make this easier.

We can simply our code by introducing two functions:

> inject :: a -> Maybe a
> inject x = Just x    -- or: inject = Just

The 'inject' functions just wraps up a value into a 'Just' type. (It is
actually identical to 'Just' but we introduce it here for didactical
purposes.)
...
that takes the unwrapped value and returns a new value of the 'Maybe' type.
Now, if the first computation fails and returns 'Nothing', this is where the
story ends. Otherwise, we unwrap the value and apply to it the function 'k'.
...
INJECT operations. The bind operation is defined as operator (>>=), while
injection operation is defined as the 'return' function.
...
Note that the type constructor 'm' is of kind '* -> *'. For instance, this
could be a 'Maybe' type constructor, or '[]', 'IO', or any user-defined type
constructor.

The instance of the 'Monad' type class for 'Maybe' type is defined in the
'Data.Maybe' module. It is defined exactly as we've defined it:
...
As a matter of fact, the 'Monad' type class has two additional functions:
...
first computation to the second computation, but instead it simply discards the
...
> v11 = Just 5 >> (Just 6 >> Just 7)
> v12 = (Just 5 >> Just 6) >> Just 7
> v13 = Just 3 >>= (\x -> Just (show x))
> v14 = Just 3 >>= (\x -> Just (show x)) >>= return . (++"!")

== MONADIC LAWS ==============================================================

Each instance of a 'Monad' should abide by the three MONADIC LAWS. This laws
are not checked by the compiler (this cannot eb done with type checking), so
you have to ensure that the laws hold. Otherwise, the instance will not be a
true monade and will not behave as expected. The three laws are:
...
a computation 'm', this is the same as letting the computation 'a' return its
value on its own. In other words, it makes no sense to take the result of a
computations just to return it. The third law is the associativity of the
binding operator (>>=). It means that the order in which we group the
...
> v16 = return 0 >>= return . (+1) :: Maybe Int
> v17 = return 0 >>= return . (+1) :: IO Int
...
Here's a revelation: the 'do' block is nothing more than a syntactic sugar. We
could have done without it, using (>>=) and 'return'.
...
we can add some suggar:

> grandmothersPartner4 :: Person -> Maybe Person
> grandmothersPartner4 p = do
>   m <- mother p
>   g <- mother m
>   partner g
