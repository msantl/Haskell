...
the function needs to (1) take a value of the 'SM' type and the initial state,
(2) unwrap the function from the monad and (3) apply the function on the
initial state.
...
unwraps our function and applies it the initial state.

In most cases we only need the return value of a computation and don't care
about the final state. We can define:
...
What is the result of the following computations?
...
We now see that a list comprehension is just a syntactic sugar for a list monad.
...
This function might seem dull at first, but it is actually quite interesting
from a theoretical point of view because it gives us an alternative definition
of a monad. Assume that 'm' is an instance of 'Functor'. This means that we have
at our disposal the 'fmap' function:
...
In other words,
instead of: unwrapping 'm a', applying 'a -> m b' and getting 'm b',
we can:     apply within 'm a' function 'a -> m b', get 'm (m b)', and then flatten
            to 'm b'.
