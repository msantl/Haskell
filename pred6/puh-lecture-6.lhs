...
LECTURE 6: Higher-order functions 2
...
Which one is the best? The third one. Beginners may resort to the first one.
The second one is simply bad.
...
of them expects only a single argument.
...
1.1.
- Define 'sumEven' that adds up elements occurring at even positions in a 
  list (zeroth element is included as well).
...
- Define 'initials3 d p s' that takes a string 's' and turns it into a string
  of initials. The function is similar to 'initial2' but additionally delimits 
...
The 'curry' function takes in a function that expects a pair and returns a
...
And so is this (though less elegant):
...
Can we define ANY recursive function using only 'map' and 'filter'?
...
We see that 'foo4' is actually 'map', while 'foo5' is filter. So, we can use
'foldr' to define 'map' and 'filter'. But the converse does not hold: FOLD is
a more general functional pattern than MAP and FILTER.
...
The other type of recursion that we considered is the one that uses the
...
5.2.
- Using 'fold' define function 'sumEven' from problem 1.1.
...
As we've seen in Lecture 4, if space complexity is an issue, we prefer tail
...
Let's look at an example of the latter case.
...
One problem with this definition is that it's time complexity. The (++)
operation is O(n), so the total time complexity of 'mapL' is O(n^2), which is
unacceptable.
...
If we want the function to generate the result lazily (and work on infinite
lists), we must use 'foldr' instead of 'foldl'. This is because 'foldr' will
start to process the list right away (using guarded recursion), while 'foldl'
must first reach the end of the list before it start's processing it. The
difference can be seen from the definitions:
...
'foldl' builds up the result in the accumulator and returns it when the lists
is empty. Because the list is infinite, this will never happen.

REMARK:

In terms of space complexity, function 'foldl' is more efficient than 'foldr'.
However, as noted in Lecture 4, we must keep in mind that Haskell is LAZY and
will not evaluate the expressions until they are needed. Because of this, we
can have MEMORY LEAKAGES even when using 'foldl'. More on this later.
